...
import { DouYin, extractDouyinUrl, getDouyinID } from '@/module/platform/douyin/index'
...
const findPlatformConfig = (msg: string, includeDisabled = false): PlatformConfig | undefined =>
  PLATFORM_CONFIG.find(config => (includeDisabled || config.enabled) && config.reg.test(msg))
...
export class kkkTools extends plugin {
   ...
   * @returns {Promise<boolean>}
   */
  async dispatchPlatform (e: CommandEvent, includeDisabled = false): Promise<boolean> {
