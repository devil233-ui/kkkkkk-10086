...
/** B站多页卡片发送进度 */
export interface DynamicPushProgressRow {
  dynamic_id: string
  host_mid: number
  groupId: string
...
export class BilibiliDBBase {
  ...
  async createTables (): Promise<void> {
    const queries = [
      ...
      `CREATE TABLE IF NOT EXISTS DynamicPushProgress (
        ...
        createdAt TEXT DEFAULT CURRENT_TIMESTAMP,
        updatedAt TEXT DEFAULT CURRENT_TIMESTAMP,
        PRIMARY KEY (dynamic_id, host_mid, groupId)
      )`,
  ...
  /** 获取多页卡片已确认的发送进度 */
  async getDynamicPushProgress (
    dynamic_id: string,
    host_mid: number,
    groupId: string
  ): Promise<DynamicPushProgressRow | null> {
    return await this.getQuery<DynamicPushProgressRow>(
      'SELECT * FROM DynamicPushProgress WHERE dynamic_id = ? AND host_mid = ? AND groupId = ?',
      [dynamic_id, host_mid, groupId]
    ) ?? null
  }

  /** 保存多页卡片已确认的发送进度 */
  async saveDynamicPushProgress (
    dynamic_id: string,
    host_mid: number,
    groupId: string,
    confirmed_pages: number,
    total_pages: number,
    last_message_id = ''
  ): Promise<DynamicPushProgressRow> {
    const now = new Date().toISOString()
  ...
  /** 删除多页卡片发送进度 */
  async deleteDynamicPushProgress (dynamic_id: string, host_mid: number, groupId: string): Promise<void> {
    await this.runQuery(
