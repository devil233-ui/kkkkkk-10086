...
import { sendBilibiliCard, withTimeout } from './cardDelivery.js'
...
const BILIBILI_CARD_SEND_TIMEOUT = 30_000
const BILIBILI_MEDIA_SEND_TIMEOUT = 180_000
        ...
        for (const target of dynamicItem.targets) {
          try {
            ...
            if (skip) {
              ...
              await bilibiliDB?.deleteDynamicPushProgress(dynamicId, dynamicItem.host_mid, target.groupId)
              continue
            }
            if (!skip) {
              ...
              if (group) {
                if (dynamicItem.dynamic_type === DynamicType.ARTICLE && articleForwardPayload) {
                  ...
                  if (forwardMessage) {
                    const articleStatus = await withTimeout(
                      () => group.sendMsg(forwardMessage as GroupSendable),
                      BILIBILI_MEDIA_SEND_TIMEOUT,
                      'B站专栏内容 sendMsg'
                    )
                    if (classifyMessageSendResult(articleStatus) === 'failed') {
                      throw new Error(`B站专栏内容发送返回明确失败: ${dynamicId}`)
                    }
                  }
                }
                if (!Array.isArray(img) || img.length === 0) {
                  throw new Error(`动态${dynamicId}未生成可发送的卡片`)
                }
                const cardResult = await sendBilibiliCard({
                  ...
                  groupId,
                  sendPage: page => group.sendMsg(page as GroupSendable),
                  timeoutMs: BILIBILI_CARD_SEND_TIMEOUT,
              ...
              if (Config.bilibili?.push?.parsedynamic) {
                switch (dynamicItem.dynamic_type) {
                  ...
                  case 'DYNAMIC_TYPE_DRAW': {
                    ...
                    if (!imgArray.length) throw new Error(`动态${dynamicId}未生成可发送的图集`)
                    ...
                    await withTimeout(
                      () => group.sendMsg(forwardMsg as GroupSendable),
                      BILIBILI_MEDIA_SEND_TIMEOUT,
                      'B站图集 sendMsg'
                    )
                    break
            ...
            if (isSendTimeoutError(e)) {
              logger.warn(`[Bilibili Push] 动态${dynamicId}发送超时，保留已确认页进度，下一轮继续`)
            } else {
              logger.error(e)
            }
          } finally {
            // 发送失败时保留页级进度，长期去重缓存只在整项成功后写入
          }
        }
      }
    } catch (e) {
