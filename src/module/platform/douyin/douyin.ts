...
const getDouyinMusicUrl = (music?: DouyinMusic): string => resolveDouyinMusicUrl(music)
    ...
    try {
      ...
      switch (this.type) {
        case 'one_work': {
          ...
          const awemeType = VideoData.data.aweme_detail.aweme_type
          ...
          if (!isArticle && VideoData.data.aweme_detail.music && hasDouyinContent('背景音乐') && !this.hasProcessedLiveImage) {
            ...
            if (music_url && this.is_mp4 === false && Config.app.removeCache === false) {
              try {
                ...
                await Common.mkdir(`${Common.tempDri.images}${cacheTitle}`)
            ...
            if (music_url && this.is_mp4 === false) {
              let audioFile: Awaited<ReturnType<typeof downloadFile>> | undefined
              try {
                ...
                if (audioFile?.filepath) {
                  await this.e.reply(await UploadRecord(
                    getUploadRecordEvent(this.e),
                    audioFile.filepath,
                    0,
                    !Config.douyin.sendHDrecord
                  ))
                }
              } finally {
