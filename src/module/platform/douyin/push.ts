...
const DOUYIN_CARD_SEND_TIMEOUT = 30_000
...
export const getDouyinMusicUrl = (music: DouyinMusic | undefined): string => resolveDouyinMusicUrl(music)
...
export class DouYinpush extends Base {
  ...
  async action (): Promise<boolean | void> {
    ...
    try {
      ...
      if (activePushTask === task) {
        activePushTask = undefined
        activePushStartedAt = 0
      }
    }
  }

  async runAction (): Promise<boolean | void> {
    try {
      ...
      for (const awemeId in data) {
         ...
         */
          ...
          for (const target of pushItem.targets) {
            ...
            if (skip) {
              // 被过滤的作品视为已处理；不存在的目标不能写缓存，否则补回机器人后也不会再推送。
              if (!pushItem.living && group) {
                await douyinDB?.addAwemeCache(actualAwemeId, pushItem.sec_uid, groupId, pushType)
              }
              continue
            }
            ...
            try {
              let status: unknown
              try {
                status = await Common.withTimeout(
                  () => group.sendMsg(img as never),
                  DOUYIN_CARD_SEND_TIMEOUT,
                  '抖音动态卡片 sendMsg'
                )
                if (isSendTimeoutError(status, true)) {
                  logger.warn(`[Douyin Push] 作品${actualAwemeId}卡片返回超时，大概率已送达，写入去重缓存后继续处理`)
                } else if (classifyMessageSendResult(status) === 'failed') {
                  throw new Error(`抖音动态卡片发送返回明确失败: ${actualAwemeId}`)
                } else if (classifyMessageSendResult(status) === 'uncertain') {
                  logger.warn(`[Douyin Push] 动态卡片未返回明确结果，按已提交处理: ${actualAwemeId}`)
                }
                cardAccepted = true
              } catch (sendError) {
                if (isSendTimeoutError(sendError, true)) {
              ...
              if (!cardAccepted) continue
              if (!pushItem.living) {
                // 卡片提交后立即去重，避免附加媒体处理卡住导致下一轮重复发送主卡片。
                await douyinDB?.addAwemeCache(actualAwemeId, pushItem.sec_uid, groupId, pushType)
              }
              ...
              if (Config.douyin?.push?.parsedynamic) {
                ...
                if (isVideo) {
                  ...
                  try {
                    ...
                    if (!imageres.length) throw new Error(`作品${actualAwemeId}未生成可发送的图集`)
                    ...
                    if (forwardMsg) {
                      await Common.withTimeout(
                        () => group.sendMsg(forwardMsg as never),
                        DOUYIN_MEDIA_SEND_TIMEOUT,
                        '抖音图集 sendMsg'
                      )
                    }
                  } finally {
              ...
              if (cardAccepted && isSendTimeoutError(error)) {
                logger.warn(`[Douyin Push] 作品${actualAwemeId}的附加媒体发送超时，卡片已去重，不再重复推送`)
