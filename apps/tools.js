...
const PLATFORM_CONFIG = [
  {
    reg: /.*((www|v|jx|jingxuan|m|live)\.(douyin|iesdouyin)\.com|douyin\.com\/(video|note|article|slides)).*/i,
    handler: 'douyin',
...
export class kkkTools extends plugin {
  ...
  async runWithErrorHandler(e, businessName, fn) {
    const handler = wrapWithErrorHandler(async event => fn.call(this, event), {
      businessName,
      plugin: this,
      retry: async () => await fn.call(this, e)
    })
