...
export class StatisticsDBBase {
  ...
  async updateDailyHistory(date, platform) {
    ...
    await this.runQuery(
      'INSERT INTO ParseHistory (date, totalParses, douyin, bilibili, xiaohongshu, createdAt) VALUES (?, 1, ?, ?, ?, ?)',
      [
        date,
        platform === 'douyin' ? 1 : 0,
  ...
  async syncHistoryFromStats() {
    ...
    for (const stat of allStats) {
      ...
      if (!dateMap.has(date)) {
        dateMap.set(date, { douyin: 0, bilibili: 0, xiaohongshu: 0 })
      }
    ...
    for (const [date, platforms] of dateMap.entries()) {
      ...
      await this.runQuery(
        'INSERT OR IGNORE INTO ParseHistory (date, totalParses, douyin, bilibili, xiaohongshu, createdAt) VALUES (?, ?, ?, ?, ?, ?)',
        [date, totalParses, platforms.douyin, platforms.bilibili, platforms.xiaohongshu, new Date().toISOString()]
      )
    }
  }
