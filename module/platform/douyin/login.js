...
let activeLoginTask = null

const getMessageId = message => message?.message_id || message?.messageId

const recallMessages = async (e, messageIds) => {
...
const getDouyinCookies = async page => {
...
const hasLoginCookies = cookies => {
  const cookieNames = new Set(cookies.map(cookie => cookie.name))
  return cookieNames.has('sid_guard') && (cookieNames.has('sessionid_ss') || cookieNames.has('sessionid'))
}

const handleSecondVerification = async (page, e, messageIds, requestVerifyCode) => {
  if (typeof requestVerifyCode !== 'function') {
    throw new Error('当前指令环境不支持接收短信验证码')
  }

  await page.waitForSelector('#uc-second-verify', { timeout: 10000 })
  const requested = await page.evaluate(() => {
    const elements = [...document.querySelectorAll('#uc-second-verify *')]
  ...
  const tipMessage = await e.reply('此次登录需要短信二次验证\n验证码已发送至抖音账号绑定的手机号，请在120秒内发送6位验证码', true)
  messageIds.push(getMessageId(tipMessage))

  for (let attempt = 1; attempt <= 2; attempt++) {
    const code = String(await requestVerifyCode() || '').trim()
    if (!/^\d{6}$/.test(code)) {
      if (attempt < 2) {
        await e.reply('验证码应为6位数字，请重新发送（剩余1次）', true)
        continue
      }
    ...
    const validateResponse = page.waitForResponse(response => response.url().includes('validate_code'), { timeout: 10000 })
    ...
    if (errorCode === 1202) {
      if (attempt < 2) {
        await e.reply('验证码错误，请重新发送（剩余1次）', true)
        continue
      }
      throw new Error('短信验证码错误次数过多')
    }

    logger.mark('[抖音登录] 短信二次验证通过，等待认证Cookie')
    return true
  }
...
const waitForQrcodeSource = async (page, timeout) => {
  await page.waitForFunction(selector => {
    const markedQrcode = document.querySelector(selector)
    if (markedQrcode?.src) return true

    return [...document.querySelectorAll('img')].some(image => {
      return image.src.startsWith('data:image/') && image.naturalWidth >= 120 && image.naturalHeight >= 120
    })
...
const findLoginButtonPoint = page => page.evaluate(() => {
...
const prepareDouyinPage = async (browser, page) => {
...
const waitForLogin = async (page, e, messageIds, requestVerifyCode) => {
  ...
  const responseHandler = async response => {
    ...
    try {
      ...
      if (setCookieHeader.includes('sid_guard')) {
        loginConfirmed = true
        logger.mark('[抖音登录] 登录响应已返回认证Cookie')
      }
      ...
      if (result?.data?.status !== 'scanned' || scanned) return

      scanned = true
      logger.mark('[抖音登录] 二维码已扫描，等待手机端确认')
      await recallMessages(e, messageIds)
      messageIds.length = 0
      const message = await e.reply('二维码已扫描，请在手机端确认登录', true)
      messageIds.push(getMessageId(message))
    } catch {
      // 轮询响应不一定都是JSON，Cookie轮询仍会继续判断登录状态。
    }
  }
  ...
  try {
    while (Date.now() < expiresAt) {
      if (secondVerifyRequired && !secondVerifyHandled) {
        ...
        await handleSecondVerification(page, e, messageIds, requestVerifyCode)
        expiresAt = Date.now() + 120 * 1000
      }

      const result = await getDouyinCookies(page)
      if (hasLoginCookies(result.cookies)) {
...
const runDouyinLogin = async (e, requestVerifyCode) => {
  const messageIds = []
  let page
  ...
  try {
    ...
    if (pageTitle.includes('验证码') || hasVerifyFrame) {
      throw new Error('抖音触发验证码中间页，请稍后重试')
    }
    ...
    const qrMessage = await e.reply([
      segment.image(`base64://${qrCodeBuffer.toString('base64')}`),
      '请在120秒内通过抖音APP扫码并确认登录'
    ], true)
    messageIds.push(getMessageId(qrMessage))

    const cookieString = await waitForLogin(page, e, messageIds, requestVerifyCode)
    if (!cookieString) {
      await recallMessages(e, messageIds)
      await e.reply('登录超时！二维码已失效，请重新扫码', true)
      return true
    }
    ...
    await recallMessages(e, messageIds)
    await e.reply(`抖音扫码登录失败：${error?.message || error}`, true)
    return true
  } finally {
    if (page) {
      await page.close().catch(error => logger.warn('[抖音登录] 页面关闭失败:', error))
    }
  }
}

export const douyinLogin = async (e, requestVerifyCode) => {
  if (activeLoginTask) {
    await e.reply('已有抖音扫码登录正在进行，请稍后再试', true)
    return true
  }

  activeLoginTask = runDouyinLogin(e, requestVerifyCode)
  try {
