 ...
 * @property {'post'|'live'} [pushType] - 推送类型
...
const VALID_DOUYIN_PUSH_TYPES = [...DEFAULT_DOUYIN_PUSH_TYPES]
...
export class DouYinpush extends Base {
  ...
  async getdata(data) {
    try {
      ...
      for (const awemeId in data) {
        ...
        const actualAwemeId = awemeId.replace(/^(post|live)_/, '')
        // 记录开始处理动态的日志信息
        logger.mark(`
        ...
        for (const target of pushItem.targets) {
          try {
            ...
            if (skip) {
              if (!pushItem.living) await douyinDB?.addAwemeCache(actualAwemeId, pushItem.sec_uid, groupId, pushType)
              continue
            }
            ...
            if (!status?.message_id) {
              logger.warn(`[Douyin Push] 动态卡片未返回 message_id，按已提交处理: ${actualAwemeId}`)
            ...
            if (error?.code === 'ETIMEDOUT' || (errStr.toLowerCase().includes('timeout') && errStr.includes('sendMsg'))) {
              ...
              logger.error(`[Douyin Push] 发送${actualAwemeId}失败:`, error)
            }
          }
        }
      }
   ...
   * @param {'post'|'live'} [pushType='post'] - 推送类型
