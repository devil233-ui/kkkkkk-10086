...
const normalizeBilibiliPushTypes = (pushTypes) => {
  if (!Array.isArray(pushTypes) || pushTypes.length === 0) return [...DEFAULT_BILIBILI_PUSH_TYPES]
  const result = []
  for (const type of pushTypes) {
    if (DEFAULT_BILIBILI_PUSH_TYPES.includes(type) && !result.includes(type)) result.push(type)
  }
  return result.length > 0 ? result : [...DEFAULT_BILIBILI_PUSH_TYPES]
}

const getDynamicParseOptions = (item) => {
  const value = item.parsedynamic !== undefined
    ? item.parsedynamic
    : Config.bilibili?.push?.parsedynamic

  if (value === true) return { video: true, draw: true }
  if (!Array.isArray(value)) return { video: false, draw: false }
  return {
    video: value.includes('视频') || value.includes('video'),
    draw: value.includes('图文') || value.includes('draw')
  }
}

export const trimBilibiliForwardDescription = (description = {}) => {
  let text = description?.text || ''
  const richTextNodes = description?.rich_text_nodes || []
  let end = 0

  for (const node of richTextNodes) {
    const nodeText = node?.orig_text || node?.text || ''
    if (!nodeText) continue
    const index = text.indexOf(nodeText, end)
    if (index !== -1) end = index + nodeText.length
  }

  if (end > 0) text = text.slice(0, end)
  return { text, richTextNodes }
}
          ...
          switch (dynamicItem.dynamic_type) {
            ...
            case DynamicType.AV: {
              if (dynamicItem.Dynamic_Data.modules.module_dynamic.major?.type === 'MAJOR_TYPE_ARCHIVE') {
                ...
                const INFODATA = await this.amagi.getBilibiliData('单个视频作品数据', { bvid, typeMode: 'strict' })
            ...
            case DynamicType.FORWARD: {
              const forwardDescription = trimBilibiliForwardDescription(dynamicItem.Dynamic_Data.modules.module_dynamic.desc)
              const text = replacetext(br(forwardDescription.text), forwardDescription.richTextNodes)
              let param = {}
            ...
            case DynamicType.ARTICLE: {
              ...
              img = await Render('bilibili/dynamic/DYNAMIC_TYPE_ARTICLE', {
                ...
                title: articleData.title || dynamicItem.Dynamic_Data.modules.module_dynamic?.major?.article?.title || 'B站专栏',
              ...
              if (parseOptions.video || parseOptions.draw) {
                switch (dynamicItem.dynamic_type) {
                  case 'DYNAMIC_TYPE_AV': {
                    if (parseOptions.video && send_video) {
                      /** @type {*} */
                      let correctList
                      let videoSize = ''
  ...
  async renderPushList() {
    ...
    for (const subscription of subscriptions) {
      ...
      renderOpt.push({
        ...
        total_favorited: Common.count(userInfo.data.data.like_num),
        following_count: Common.count(userInfo.data.data.card.attention),
        pushTypes: normalizeBilibiliPushTypes(configItem?.pushTypes).map(type => BILIBILI_PUSH_TYPE_LABELS[type] || type).join(' / ')
      })
    }
