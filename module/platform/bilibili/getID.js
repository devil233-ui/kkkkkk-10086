...
export const getBilibiliID = async (url, log = true) => {
  ...
  try {
    ...
    const shortLinkHosts = new Set(['b23.tv', 'bili2233.cn'])
    const isShortLink = shortLinkHosts.has(new URL(longLink).hostname.toLowerCase())
    ...
    if (isShortLink) {
      try {
        ...
        // 短链的目标就在 302 Location 中，无需跟随跳转并下载最终页面。
        for (let redirectCount = 0; redirectCount < 3; redirectCount++) {
          const currentUrl = new URL(longLink)
          if (!shortLinkHosts.has(currentUrl.hostname.toLowerCase())) break

          const tempLink = await new Networks({
            url: longLink,
            headers: customHeaders,
            maxRetries: 0
          }).getLocation()

          if (!tempLink || tempLink === longLink) break
          longLink = new URL(tempLink, longLink).href
        }
      } catch (error) {
        logger.warn(`[B站短链解析] 重定向超时或被拦截：${error.message}`)
      }
    }
        ...
        (url) => {
          try {
            ...
            return (
              ...
              (hostname === 'm.bilibili.com' && /^\/(?:dynamic|opus)\/\d+/.test(pathname))
            )
        ...
        (url) => {
          try {
            ...
            const match = /^https:\/\/(?:t|www)\.bilibili\.com\/(?:opus\/)?(\d+)/.exec(url) ||
              ...
              (hostname === 'm.bilibili.com' && pathname.match(/^\/(?:dynamic|opus)\/(\d+)/))
